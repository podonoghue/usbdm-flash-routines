...
void programRange(FlashData_t *flashData) {
   ...
   prepareSectors((flashData->address)>>12, (flashData->address + flashData->dataSize-1)>>12);
   while (size>0) {
      ...
      copyRamtoRom(address, data, blockSize, flashData->frequency);
      address += blockSize;
      size    -= blockSize;
   }
...
void eraseRange(FlashData_t *flashData) {
   ...
   eraseSectors(startSector, endSector, flashData->frequency);
...
static const uint8_t buffer[256] = {
      0x33,0x00,0x11,0x22,0x33,0x44,0x55,0x66,0x77,0x88,0x99,0xAA,0xBB,0xCC,0xDD,0xEE,0xFF,
...
static const FlashData_t flashdataB = {
      /* flags      */ DO_INIT_FLASH|DO_PROGRAM_RANGE|DO_VERIFY_RANGE,
      /* controller */ IAP_ADDRESS_PTR,
      /* frequency  */ RESET_CLOCK_FREQ_kHz,
      /* errorCode  */ 0xAA55,
      /* sectorSize */ FLASH_SECTOR_SIZE,
